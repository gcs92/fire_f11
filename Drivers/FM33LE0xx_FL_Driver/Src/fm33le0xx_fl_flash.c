...
FL_ErrorStatus FL_FLASH_PageErase(FLASH_Type *FLASHx, uint32_t address)
{
	...
	FL_FLASH_SetFlashLowRegionLock(FLASHx,FL_FLASH_BLOCK_15,FL_FLASH_LOCK_NONE);
    /* 擦请求 */
    *((uint32_t *)address) =  FL_FLASH_ERASE_REQUEST;
    while(1)
    {
        ...
        else if(FL_FLASH_IsActiveFlag_EraseComplete(FLASHx))
        {
            /*编程成功*/
            FL_FLASH_ClearFlag_EraseComplete(FLASHx);
            ret =  FL_PASS;
...
FL_ErrorStatus FL_FLASH_Program_Word(FLASH_Type *FLASHx, uint32_t address, uint32_t data)
{
    ...
    *((uint32_t *)address) =  data;
    while(1)
    {
        ...
        else if(FL_FLASH_IsActiveFlag_ProgramComplete(FLASHx))
        {
            /*编程成功*/
...
/**
  ...
  * @param  FLASHx 外设入口地址
  * @param  PageNum 为需要编程的已经擦除过的扇区号，FM33LE02最大为128，非对齐地址编程将触发fault。
  * @param  *data 待写入页数据
  * @retval 错误状态，可能值：
  *         -FL_FAIL 编程发生错误
  *         -FL_PASS 编程成功
