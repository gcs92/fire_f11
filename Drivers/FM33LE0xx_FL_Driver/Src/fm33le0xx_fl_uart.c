...
#define         IS_FL_UART_CLKSRC(__VALUE__)            (((__VALUE__) == FL_RCC_UART2_CLK_SOURCE_APB1CLK)||\
                                                         ((__VALUE__) == FL_RCC_UART2_CLK_SOURCE_RCHF)||\
                                                         ((__VALUE__) == FL_RCC_UART2_CLK_SOURCE_SYSCLK)||\
                                                         ((__VALUE__) == FL_RCC_UART2_CLK_SOURCE_RCMF_PSC)||\
                                                         ((__VALUE__) == FL_RCC_UART1_CLK_SOURCE_APB1CLK)||\
...
FL_ErrorStatus FL_UART_DeInit(UART_Type *UARTx)
{
    ...
    {
        /*复位UART*/
        FL_RCC_EnableResetAPB2Peripheral(FL_RCC_RSTAPB_UART2);
        FL_RCC_DisableResetAPB2Peripheral(FL_RCC_RSTAPB_UART2);
        /* 外设总线时钟关闭 */
        FL_RCC_DisableGroup3BusClock(FL_RCC_GROUP3_BUSCLK_UART2);
        /* 外设操作时钟关闭 */
        FL_RCC_DisableGroup1OperationClock(FL_RCC_GROUP1_OPCLK_UART2);
    }
    else if(UARTx == UART4)
    {
        /*复位UART*/
...
FL_ErrorStatus FL_UART_Init(UART_Type *UARTx, FL_UART_InitTypeDef *initStruct)
{
    ...
    {
        /*时钟源选择*/
        FL_RCC_SetUART2ClockSource(initStruct->clockSrc);
        /* 根据不同的时钟源计算baudrate 寄存器值,并配置 */
        switch(initStruct->clockSrc)
        {
            case FL_RCC_UART2_CLK_SOURCE_APB1CLK:
                Fclk = FL_RCC_GetAPB1ClockFreq();
                break;
            case FL_RCC_UART2_CLK_SOURCE_RCHF:
                Fclk = FL_RCC_GetRCHFClockFreq();
                break;
            case FL_RCC_UART2_CLK_SOURCE_SYSCLK:
                Fclk = FL_RCC_GetSystemClockFreq();
                break;
            case FL_RCC_UART2_CLK_SOURCE_RCMF_PSC:
                Fclk = FL_RCC_GetRCMFClockFreq();
                break;
        }
        baudRate = Fclk / initStruct->baudRate - 1;
    ...
    {
        /*总线时钟使能*/
        FL_RCC_EnableGroup3BusClock(FL_RCC_GROUP3_BUSCLK_UART2);
        /*操作时钟使能*/
        FL_RCC_EnableGroup1OperationClock(FL_RCC_GROUP1_OPCLK_UART2);
    }
    else if(UARTx == UART4)
    {
        /*总线时钟使能*/
